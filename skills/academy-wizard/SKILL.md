...
**Use an honest syllabus-led Slides home.** For multi-SCO courses, omit module-tile status labels, completion coloring and cross-SCO progress reads. Use the maintained Grid Disturbance-style two-column tiles (one column on mobile), retaining titles, subtitles and numbered badges. Without an approved course-link navigation override, render tiles as informational articles: no link, pointer cursor, hover lift or keyboard tab stop. Put the navigation instruction directly below the tiles at normal tile body-text size (0.85rem); for the approved Docebo left-sidebar treatment use exactly “Navigate modules in any order using the Syllabus in the left sidebar.” Do not imply a sidebar location or unrestricted order for an LMS where these have not been confirmed. When the user requests a first-module start action, place “Start with MODULE 1” in OCP indigo (#343895) to the left of the same 46px circular double-chevron control used on non-final module endings, below the instruction. Record `index_module_links`, `index_navigation_instruction` and the opt-in `index_start` in `course.json`; see `references/course_schema.md`. A direct first-module link is an explicit compatibility choice, not a standard SCORM 1.2 navigation command. Do not silently extend it to arbitrary module tiles or claim correct LMS tracking from a browser-only preview. Keep Scrolling and explicitly approved single-SCO progress behavior unchanged. Stop after the review build when ZIP approval is pending, and preserve these shared changes for the eventual PR.
