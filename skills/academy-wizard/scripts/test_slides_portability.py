...
class SlidesPortabilityTests(unittest.TestCase):
    ...
    def test_local_preview_persists_and_clears_quiz_state_per_module(self):
        course={'style':'Slides','course_slug':'resume-test','course_title':'Resume test',
                'modules':[{'id':2,'title':'Module two','slides':[
                    {'id':1,'type':'title','title':'Module two'},
                    {'id':2,'type':'course_complete'}]}]}
        result=render_module(course,0)
        self.assertIn("const QUIZ_STATE_KEY = \"ocp:resume-test:module2:quiz-state\"",result)
