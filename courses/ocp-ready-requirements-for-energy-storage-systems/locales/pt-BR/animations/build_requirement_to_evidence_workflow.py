...
STRINGS = {
    "language": "en-US",
    "title": "From Use Case to Approval-Ready Design",
    "subtitle": "Each decision produces the evidence needed for the next.",
    "steps": [
        {"label": "Use case", "detail": "business need"},
        {"label": "Deployment", "detail": "location + topology"},
        {"label": "Hazards", "detail": "credible risks"},
        {"label": "Standards", "detail": "applicable stack"},
        {"label": "Early review", "detail": "AHJ + insurer"},
        {"label": "Evidence", "detail": "tests + plans"},
        {"label": "Operations", "detail": "procedures"},
        {"label": "OCP feedback", "detail": "lessons learned"},
    ],
    "loop": "Continuous community learning",
    "brand": "OCP Academy",
}


def build_steps(strings: dict) -> list[tuple]:
    accents = [INK, GREEN_DARK, RED, INK, AMBER, GREEN_DARK, INK, GREEN_DARK]
    positions = [
        (XS[0], TOP_Y),
        (XS[1], TOP_Y),
        (XS[2], TOP_Y),
    ...
    return [
        (
            index + 1,
            strings["steps"][index]["label"],
            strings["steps"][index]["detail"],
            positions[index][0],
            positions[index][1],
            accents[index],
        )
...
def draw_card(base: Image.Image, step: tuple, reveal: float, active: bool, pulse: float) -> None:
    ...
    label_font = fitted_font(FONT_BOLD, 24, label, CARD_W - 84, 12)
    detail_font = fitted_font(FONT_REGULAR, 17, detail, CARD_W - 84, 10)
...
def make_frame(t: float) -> Image.Image:
    ...
    title_font = fitted_font(FONT_BOLD, 40, STRINGS["title"], WIDTH - 110, 24)
    subtitle_font = fitted_font(FONT_REGULAR, 21, STRINGS["subtitle"], WIDTH - 114, 14)
    draw.text((55, 44), STRINGS["title"], font=title_font, fill=INK)
    draw.text((57, 99), STRINGS["subtitle"], font=subtitle_font, fill=MUTED)
    ...
    if loop_alpha:
        ...
        loop_font = fitted_font(FONT_BOLD, 18, STRINGS["loop"], 270, 11)
        loop_draw.text((82, 630), STRINGS["loop"], font=loop_font, fill=(52, 56, 149, loop_alpha))
        base.alpha_composite(layer)

    draw = ImageDraw.Draw(base)
    draw.text((982, 659), STRINGS["brand"], font=STEP_FONT, fill=INK)
    draw.rectangle((1173, 659, 1225, 665), fill=GREEN)
    return base.convert("RGB")


def main() -> None:
    global STRINGS, STEPS
    ...
    parser.add_argument(
        "--strings-json",
        type=Path,
        help="Localized animation strings. Defaults to locale_strings.json next to this script when present.",
    )
    args = parser.parse_args()
    strings_path = args.strings_json
    if strings_path is None:
        candidate = Path(__file__).resolve().parent / "locale_strings.json"
        if candidate.exists():
