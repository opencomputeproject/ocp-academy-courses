...
REGULAR_FONT = Path("/System/Library/Fonts/STHeiti Medium.ttc")
...
def build_frame(time_seconds: float) -> Image.Image:
    ...
    if time_seconds < 1.6:
        stage_index = 0
        stage_label = "1  標準 ETHERNET / IP 幀"
    elif time_seconds < 3.8:
        stage_index = 1
        stage_label = "2  找出通用額外負荷"
    elif time_seconds < 5.9:
        stage_index = 2
        stage_label = "3  取代 IP HEADER"
    else:
        stage_index = 3
        stage_label = "4  保留縱向擴展所需控制"
