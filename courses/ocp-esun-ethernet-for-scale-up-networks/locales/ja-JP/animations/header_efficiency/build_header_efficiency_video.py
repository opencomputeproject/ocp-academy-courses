...
def build_frame(time_seconds: float) -> Image.Image:
    ...
    draw.text(
        (72, 80),
        "How ESUN reclaims packet overhead",
        font=font(40, True),
        fill=rgba(GRAY_DARK, content_alpha),
    )
    draw.text(
        (74, 132),
        "The payload stays the same. The network header becomes scale-up specific.",
        font=font(23),
    ...
    if time_seconds < 1.6:
        stage_index = 0
        stage_label = "1  STANDARD ETHERNET / IP FRAME"
    elif time_seconds < 3.8:
        stage_index = 1
        stage_label = "2  IDENTIFY GENERAL-PURPOSE OVERHEAD"
    elif time_seconds < 5.9:
        stage_index = 2
        stage_label = "3  REPLACE THE IP HEADER"
    else:
        stage_index = 3
        stage_label = "4  KEEP THE CONTROLS SCALE-UP NEEDS"
    ...
    segment_specs = [
        ...
        ((233, 236, 239), "Transport + Payload\nunchanged", GRAY_DARK, 22),
    ...
    pre_replace_text = (
        "General-purpose routing and control fields"
        if time_seconds < 3.8
        else "Keep only what the scale-up domain needs"
    )
    ...
    draw_centered(
        draw,
        (640, 566),
        "Ethernet framing stays. Header overhead falls.",
        font(29, True),
    ...
    draw_centered(
        draw,
        (640, 612),
        "20–40 B removed  ·  4 B added back",
        font(22, True),
